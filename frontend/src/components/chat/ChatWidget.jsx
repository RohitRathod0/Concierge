...
const WIDGET_STORAGE_KEY = 'et_ai_widget_state_v1';

const PROMPTS_BY_PATH = {
  '/': {
    id: 'sys-1',
    delay: 8000,
...
function mapCrossSellToWidgetCard(crossSell) {
  if (!crossSell) return null;

  return {
    headline: crossSell.service,
    subtext: crossSell.why_for_you,
    cta_text: crossSell.cta || 'Learn More',
...
export default function ChatWidget() {
  const persistedState = loadWidgetState();
  ...
  const [hasPrompted, setHasPrompted] = useState(persistedState?.hasPrompted ?? false);
  const [sessionId, setSessionId] = useState(persistedState?.sessionId ?? null);
  ...
  }, [isOpen, messages, hasPrompted, sessionId]);

  useEffect(() => {
    const promptConfig = PROMPTS_BY_PATH[location.pathname];
    if (!promptConfig || hasPrompted || messages.length > 0) {
      return;
    }
  ...
  }, [location.pathname, hasPrompted, messages.length]);
              ...
              <div key={m.id} className={`flex w-full flex-col ${m.sender === 'user' ? 'items-end' : m.sender === 'sys' ? 'items-center' : 'items-start'}`}>
                ...
                {m.recommendation && (
                  <div className="mt-2 w-[85%] bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200 rounded-xl p-4 shadow-sm relative overflow-hidden group">
                    ...
                    <button
                      onClick={() => navigate(m.recommendation.cta_url)}
                      className="w-full bg-slate-900 hover:bg-black text-white text-sm font-bold py-2.5 rounded-lg transition-colors flex justify-center items-center gap-2 shadow-md"
                    >
